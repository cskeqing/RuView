...
export default class ModelPanel {
  ...
  async _load(id) {
    ...
    catch (e) { this._set({ loading: false, error: i18n.t('model.loadFailed', {msg: e.message}) }); }
  }

  async _unload() {
    this._set({ loading: true, error: null });
    try { await modelService.unloadModel(); await this.refresh(); }
    catch (e) { this._set({ loading: false, error: i18n.t('model.unloadFailed', {msg: e.message}) }); }
  }

  async _delete(id) {
    this._set({ loading: true, error: null });
    try { await modelService.deleteModel(id); await this.refresh(); }
    catch (e) { this._set({ loading: false, error: i18n.t('model.deleteFailed', {msg: e.message}) }); }
  }
  ...
  async _loraChange(modelId, profile) {
    ...
    catch (e) { this._set({ loading: false, error: i18n.t('model.loraFailed', {msg: e.message}) }); }
  }
  ...
  render() {
    ...
    hdr.appendChild(this._el('span', 'mp-title', i18n.t('model.title')));
    ...
    ls.appendChild(this._el('div', 'mp-section-title', i18n.t('model.available')));
    ...
    if (models.length === 0 && !this.state.loading) {
      ls.appendChild(this._el('div', 'mp-empty', i18n.t('model.empty')));
    } else {
    ...
    const rb = this._btn(i18n.t('model.refresh'), 'mp-btn mp-btn-secondary', () => this.refresh());
  ...
  _renderActive() {
    ...
    card.appendChild(this._el('div', 'mp-active-name', am.model_id || i18n.t('model.activeModel')));
    ...
    if (am.avg_inference_ms != null) {
      ...
      st.innerHTML = `<span class="mp-stat-label">${i18n.t('model.inference')}</span> <span class="mp-stat-value">${am.avg_inference_ms.toFixed(1)} ms</span><span class="mp-stat-sep">|</span><span class="mp-stat-label">${i18n.t('model.frames')}</span> <span class="mp-stat-value">${am.frames_processed ?? 0}</span>`;
      card.appendChild(st);
    }

    if (this.state.loraProfiles.length > 0) {
      const row = this._el('div', 'mp-lora-row');
      row.appendChild(this._el('span', 'mp-lora-label', i18n.t('model.loraProfile')));
      const sel = document.createElement('select');
      sel.className = 'mp-lora-select';
      const def = document.createElement('option');
      def.value = ''; def.textContent = i18n.t('model.none'); sel.appendChild(def);
    ...
    const ub = this._btn(i18n.t('model.unload'), 'mp-btn mp-btn-danger', () => this._unload());
  ...
  _renderCard(model) {
    ...
    const lb = this._btn(i18n.t('model.load'), 'mp-btn mp-btn-success', () => this._load(model.id));
    lb.disabled = this.state.loading;
    const db = this._btn(i18n.t('model.delete'), 'mp-btn mp-btn-muted', () => this._delete(model.id));
